/*
	Name: Ziad Eliwa
	ID : 900246124
	This crawling script was written in Go programming language
	using Colly for HTML Parsing, ChromeDP for JS Rendering and simulating a browser.

	For running the code run in the terminal $ go run scraperscript.go locater.go

	This is my email ziadeliwa@aucegypt.edu if there is inquiries about the code.
	I have tried as much as I can to add comments to make this easier to read.
	Sorry for not using python :)
*/
...
// File writer function to help in populating csv files.
...
// Using ChromeDP to Render the page before passing it to Colly.
...
func main() {
	...
	*/
	...
	// This is a collector that parses the html as soon it receives a response.
	categoryc := colly.NewCollector()
	categorywriter, category_file := fileWriter("csv/category.csv")
	defer category_file.Close()
	defer categorywriter.Flush()

	categoryc.OnHTML("section", func(e *colly.HTMLElement) {
		// finding all the category types
		title := e.ChildText("h3")
		e.ForEach("ul li", func(i int, h *colly.HTMLElement) {
			// extracting all the attributes of that category.
			data := h.Text
			var str string
	...
	categoryc.OnRequest(func(r *colly.Request) {
		// passing the rendered html.
	...
	*/
	...
	// Collecting links of all platforms.
	...
	// Loops over platform links to extract 
	...
	*/
	...
	// These are collectors that parse the html as soon as they receive a response.
	...
	// looping over the game links to extract each game information.
	...
	*/
	...
	// Giving companies collector CSS selectors to parse the HTML.
	var title string
	...
	// Looping over the companies links to extract the information from them.
	for i := 0; i < len(company_urls); i++ {
		...
		// Gathering Company details in a string array
	...
	// User Faking 
	Userwriter, Userfile := fileWriter("csv/User.csv")
	defer Userfile.Close()
	defer Userwriter.Flush()

	UserEmailswriter, UserEmailsfile := fileWriter("csv/UserEmails.csv")
	defer UserEmailsfile.Close()
	defer UserEmailswriter.Flush()


	var username,birthdate,gender,country string
	f := faker.New()
	for i := 0; i < 300; i++ {
		username = f.Internet().User()
		
		start := time.Date(1970, 1, 1, 0, 0, 0, 0, time.UTC)
		end := time.Date(2005, 12, 31, 23, 59, 59, 0, time.UTC)
		birth := f.Time().TimeBetween(start, end)
		birthdate = birth.Format("2006-01-02")
		country = f.Address().Country()
		gender = randomGender()
		
		Userwriter.Write([]string{username,birthdate,gender,country})
		Userwriter.Flush()

		for i := 0; i <= rand.Intn(3); i++ {
			UserEmailswriter.Write([]string{username,f.Internet().Email()})
		}
	}
}
